...
notify_telegram() {
    ...
    -d chat_id=${chat_id} \
    -d text="Your Deep Learning Masking slurm job (Job ID: $SLURM_JOB_ID) has completed with status: $status"
}

trap 'notify_telegram "FAILED (job terminated or timed out)"' TERM EXIT

if [ -z "$VIRTUAL_ENV" ]; then
    source venv_DL/bin/activate || { echo "Failed to activate virtual environment"; exit 1; }
fi

ulimit -a

make mask || { echo "Mask creation failed"; exit 1; }
